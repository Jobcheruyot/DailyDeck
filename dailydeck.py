...
fig.update_layout(
    title="<b>Global Day vs Night Sales — Only Stores with NIGHT Shifts</b>",
    title_x=0.5,
    showlegend=True,
    legend_title_text="Shift (with %)",
    margin=dict(l=20, r=20, t=60, b=20),
...
# ============================================
# 2nd-HIGHEST CHANNEL SHARE — TOP & BOTTOM 30
# ============================================
...
print(f"✅ New column 'CUST_CODE' created successfully.")
print(f"🧾 Total unique receipts across all stores: {unique_receipts:,}")
...
# @title
# ✅ Count unique CUST_CODEs per store (customer/receipt counts)
...
# 🧾 Merge the counts into the existing sales_summary table
...
# ==============================================
# 🧹 1) Clean + Coerce numeric columns
# ==============================================
def _to_num(s):
...
# ------------------------------------------------
# 3) Build complete 30-min grid (00:00 → 23:30)
# ------------------------------------------------
...
def build_branch_pivot(branch_name: str):
    ...
    # 👇 Total customers for that store (unique CUST_CODE)
...
# 🧼 Ensure TRN_DATE is datetime
df['TRN_DATE'] = pd.to_datetime(df['TRN_DATE'], errors='coerce')

# 🏪 Create CASHIER-COUNT if not exists
if 'CASHIER-COUNT' not in df.columns:
    df['CASHIER-COUNT'] = df['CASHIER'].astype(str).str.strip() + '-' + df['STORE_NAME'].astype(str).str.strip()

# 🖥️ Ensure Till_Code exists (combine TILL and STORE_CODE)
if 'Till_Code' not in df.columns:
    df['TILL'] = df['TILL'].astype(str).fillna('').str.strip()
    df['STORE_CODE'] = df['STORE_CODE'].astype(str).fillna('').str.strip()
    df['Till_Code'] = df['TILL'] + '-' + df['STORE_CODE']
...
# ✅ Count unique CASHIER-COUNT per STORE_NAME
...
# 🪄 Reset index and format numbering to start from 1
...
# =====================================
# 2️⃣ Receipt-level duration (min) and item count
# =====================================
receipt_duration = (
...
# =====================================
# 4️⃣ Cashier summary (duration + customer count)
# =====================================
cashier_durations = (
...
# =======================================================
# 3) Build full 30-minute time grid (00:00 → 23:30)
# =======================================================
start_time = pd.Timestamp("00:00:00")
intervals = [(start_time + timedelta(minutes=30*i)).time() for i in range(48)]
...
def build_heatmap(branch):
    ...
    fig = px.imshow(
        ...
        color_continuous_scale='Blues',
        aspect='auto',
        title=f"🕒 Half-Hourly Till Activity — {branch}",
        text_auto=False
    )
...
for b in branches:
    ...
    buttons.append(dict(
        ...
        args=[
            ...
            {
                'title': f"🕒 Half-Hourly Till Activity — {b}"
            }
        ]
    ))
...
# =======================================================
# 2️⃣ Aggregate: unique receipts per till every 30 mins
# =======================================================
till_activity = (
    df.groupby(['STORE_NAME', 'Till_Code', 'TIME_ONLY'], as_index=False)
      .agg(Receipts=('CUST_CODE', 'nunique'))
...
# =======================================================
# 3️⃣ Build full 30-minute grid (00:00 → 23:30)
# =======================================================
start_time = pd.Timestamp("00:00:00")
intervals = [(start_time + timedelta(minutes=30*i)).time() for i in range(48)]
...
# =======================================================
# 4️⃣ Summary metrics (with new columns)
# =======================================================
branch_summary = (
    till_activity.groupby('STORE_NAME', as_index=False)
        .agg(
            ...
            Avg_Per_Till=('Receipts','mean'),
            Max_Per_Till=('Receipts','max'),
            Unique_Tills=('Till_Code','nunique')  # ✅ New column
        )
)
...
# =======================================================
# 5️⃣ Interactive heatmap with dropdown (far right)
# =======================================================
...
# =======================================================
# 6️⃣ Display Final Summary Table (with Unique Tills)
# =======================================================
pd.set_option('display.max_columns', None)
pd.set_option('display.width', 1500)
...
def build_compliance_plot(branch):
    ...
    # ✅ Non-compliant bars (red) with visible counts (even if 0)
    fig.add_bar(
        y=pivot.index,
...
for b in branches:
    ...
    buttons.append(dict(
        ...
        args=[
            ...
            {
                'title': f"🏪 Tax Compliance by Till — {b}",
                'annotations': []
            }
        ]
    ))
...
fig_store.update_layout(
    updatemenus=[dict(
        type='dropdown',
        x=1.34, xanchor='left',   # 👉 far right placement
        y=1.15, yanchor='top',
        buttons=buttons,
        showactive=True,
        direction='down',
...
def run_analysis(_=None):
    ...
    # ===========================================
    # 🟦🟩 Chart (commas + legend as color key)
    # ===========================================
    fig = px.bar(
        combined,
        ...
        title=f"🏆 Top {top_x} Items by {metric} — {selected_branch} vs Global"
    )
    ...
    if len(missing_df) > 0:
        fig_missing = px.bar(
            ...
            orientation='h',
            text='Count_of_Baskets',
            title=f"⚠️ Items in Global Top {top_x} but Missing/Underperforming in {selected_branch}",
        ...
        print(f"✅ All top {top_x} global items are also among {selected_branch}'s top performers.")

    # ===========================================
    # 🧾 Numbered Tables (1..N) with commas
    # ===========================================
    print("\n🔹 Global Top Items:")
    display(fmt_table(global_top[['#','ITEM_NAME','Count_of_Baskets','QTY','NET_SALES']])
            .style.background_gradient(cmap='Blues'))

    print(f"\n🔹 {selected_branch} Top Items:")
    display(fmt_table(branch_top[['#','ITEM_NAME','Count_of_Baskets','QTY','NET_SALES']])
            .style.background_gradient(cmap='Greens'))

    print(f"\n⚠️ Items Missing or Underperforming in {selected_branch}:")
...
# =======================================================
# 🏬 Category Contribution Across All Branches (Numbered, % table + alpha-sorted chart)
# =======================================================
import pandas as pd
import numpy as np
import plotly.express as px
...
pd.set_option('display.max_columns', None)
pd.set_option('display.width', 1500)
print("📊 Category Contribution (%) by Branch (numbered, with TOTAL row)")
display(fmt_tbl)
...
# =======================================================
# 🧺 Category Contribution Across Branches — Based on Baskets (Unique Receipts)
# =======================================================
import pandas as pd
import numpy as np
import plotly.express as px
...
# --- 2️⃣ Compute per-branch, per-category unique basket counts ---
summary_baskets = (
...
# Compute GLOBAL TOTAL row — % of all baskets contributed by each category
...
pd.set_option('display.max_columns', None)
pd.set_option('display.width', 1500)
print("📊 Category Contribution by Baskets (%) per Branch (numbered, with TOTAL row)")
display(fmt_tbl)

# --- 5️⃣ Visualization: stacked horizontal bar (branches alphabetical) ---
...
fig = px.bar(
    ...
    color='CATEGORY',
    orientation='h',
    title='🧺 Category Contribution by Branch (Based on Baskets)',
    color_discrete_sequence=px.colors.qualitative.Safe
)
...
# =======================================================
# 🧺 Supplier basket share with scoped filters
...
def _render(_=None):
    with out:
        ...
        print("📊 Supplier share of baskets within filter scope")
        ...
        fig = px.bar(
            ...
            title=f"{supplier} — Share of Baskets (within filter scope)"
        )
...
# =======================================================
# 🧺 Supplier contribution (% of baskets) by Category → Department → Branch
# (✅ Single filter row — no duplication)
# =======================================================
import pandas as pd
import numpy as np
import plotly.express as px
...
def _render(_=None):
    with out:
        ...
        print("📊 Supplier Contribution — % of Baskets within scope")
...
def build_zero_table(zero_df, source_label, target_label, metric):
    """
    Presentable table for “Top in <source> but ZERO in <target>”
...
run_btn = widgets.Button(description='🔍 Compare', button_style='success', layout=widgets.Layout(width='140px', height='36px'))
...
def run(_=None):
    ...
    fig = px.bar(
        ...
        title=f"🏬 Branch Comparison — Top {top_x} by {metric}: {a} vs {b}"
    )
    fig.update_traces(texttemplate='%{text:,}', textposition='outside', cliponaxis=False)
    fig.update_layout(
    ...
    plot_zero(tbl_A_zero_in_B, f"⚠️ Top in {a} but ZERO in {b} (baskets/qty/sales)")
    plot_zero(tbl_B_zero_in_A, f"⚠️ Top in {b} but ZERO in {a} (baskets/qty/sales)")
    plot_zero(tbl_GLOBAL_zero_in_A, f"🌍 Top Globally but ZERO in {a} (baskets/qty/sales)")
    plot_zero(tbl_GLOBAL_zero_in_B, f"🌍 Top Globally but ZERO in {b} (baskets/qty/sales)")

    # ===== Top-X tables for each branch =====
    print(f"\n🔹 {a} — Top {top_x} by {metric}:")
    display(topA[['#','ITEM_NAME','Count_of_Baskets','QTY','NET_SALES']])

    print(f"\n🔹 {b} — Top {top_x} by {metric}:")
    display(topB[['#','ITEM_NAME','Count_of_Baskets','QTY','NET_SALES']])

    print(f"\n🔹 🌍 GLOBAL — Top {top_x} by {metric}:")
    display(topGLOBAL[['#','ITEM_NAME','Count_of_Baskets','QTY','NET_SALES']])

    # ===== ZERO-in-other tables (ALWAYS DISPLAY) =====
    print(f"\n🧭 Top in {a} but ZERO in {b}:")
    display(fmt_table(tbl_A_zero_in_B))

    print(f"\n🧭 Top in {b} but ZERO in {a}:")
    display(fmt_table(tbl_B_zero_in_A))

    print(f"\n🧭 🌍 Top Globally but ZERO in {a}:")
    display(fmt_table(tbl_GLOBAL_zero_in_A))

    print(f"\n🧭 🌍 Top Globally but ZERO in {b}:")
...
item_options = [(f"{row.ITEM_CODE} — {row.ITEM_NAME}", row.ITEM_CODE) for _, row in item_lookup.iterrows()]

item_dd = widgets.Combobox(
    options=[opt[0] for opt in item_options],
    placeholder='Type or pick an ITEM_CODE — ITEM_NAME',
    description='SKU:',
    ensure_option=False,
...
run_btn = widgets.Button(
    description='🔎 Analyze', button_style='success',
...
def analyze_selected_item(_=None):
    ...
    target_code = raw.split('—', 1)[0].strip() if '—' in raw else raw
    if target_code not in set(df['ITEM_CODE']):
        print(f"❌ ITEM_CODE '{target_code}' not found.")
        return
    ...
    # Display Results
    # ----------------------------
    print(f"🧺 Basket Composition for SKU {target_code} — {any_name}")
    print(f"• CATEGORY: {item_cat}   • DEPARTMENT: {item_dept}\n")
    ...
    if show_chart_chk.value:
        ...
        fig = px.bar(
            ...
            title=f"Stores — Basket Split (%) for {target_code} ({any_name})"
        )
        fig.update_layout(
            barmode='stack',
...
print("🌍 All-Branch Overview — Loyalty Customers with >1 Baskets (table only)")
...
def render_branch_table(_=None):
    with branch_out:
        ...
        # format & display
        if per_store.empty:
            print(f"🟢 No loyalty customers with more than one basket in {branch}.")
            return
        ...
        print(f"🏪 Branch Overview — {branch} (Loyal customers with >1 baskets)  |  "
...
def render_customer_detail(_=None):
    with cust_out:
        ...
        if not sel_cust:
            print("🔍 Select a loyalty customer (with >1 baskets globally) to view receipts across stores.")
            return

        rc = receipts[receipts['LOYALTY_CUSTOMER_CODE'] == sel_cust].copy()
        if rc.empty:
            print(f"❌ No receipts found for loyalty customer '{sel_cust}'.")
            return
        ...
        print(f"👤 Loyalty Customer — {sel_cust}")
        print("📌 Per-Store Summary (All Stores)")
        display(per_store_disp[['#','STORE_NAME','Baskets','Total_Value','First_Time','Last_Time']])

        print("🧾 Receipt-Level Detail (All Stores)")
...
# =======================================================
# 🧾 High-Level Store Summary — Multi-Priced SKUs per Day
...
# 1️⃣ --- Compute per (store, date, item) ---
grp = (
...
# 2️⃣ --- Aggregate high-level summary per store (ONLY where spread > 0)
summary = (
...
# 3️⃣ --- Add TOTAL row (sum + max rules)
total_row = pd.DataFrame({
    '#': [''],
    'STORE_NAME': ['TOTAL'],
...
print("🏬 High-Level Store Summary — Items Sold at Multiple Prices per Day (Spread > 0 only)")
display(fmt[['#','STORE_NAME','Items_with_MultiPrice','Total_Diff_Value','Avg_Spread','Max_Spread']])

# 5️⃣ --- Visualization: Top Stores by Value Difference (spread > 0)
...
if topN > 0:
    ...
    print("✅ No multi-priced items with positive spread found across stores.")

"""##Branch Brach Overview"""

# =======================================================
# 🔎 Branch Drilldown (clean view): All SKUs with >1 price in a day
...
def render(_=None):
    with out:
        ...
        if multi.empty:
            print(f"✅ {store}: No SKUs with more than one distinct price (spread > 0) on the same day.")
            return
        ...
        print(f"• Item-Days with >1 price (spread>0): {sku_days:,}   • Distinct SKUs affected: {sku_count:,}   • Total Diff Value: {value_sum:,.2f}\n")

        print("🧾 Per Item/Day — Summary (min/max price, spread, total qty, diff value)")
        ...
        price_view['Time_Window'] = price_view['First_Time'] + ' → ' + price_view['Last_Time']
        ...
        print("🔍 Detailed — Price Breakdown (clean view)")
...
# ======================================================
# 2️⃣ Add TOTAL per Store and Grand Total
# ======================================================
store_totals = (
...
store_totals['CAP_CUSTOMER_CODE'] = '—'
...
def render(_=None):
    with out:
        ...
        title = "🧾 Negative Receipts Summary by Branch & CAP_CUSTOMER_CODE"
...
# ============================
# 🔎 Negative Receipts — Summed per Unique Receipt
# ============================
import pandas as pd
import numpy as np
import ipywidgets as widgets
...
# ---- Validate & prep ----
need = ['STORE_NAME', 'CUST_CODE', 'TRN_DATE', 'NET_SALES']
missing = [c for c in need if c not in df.columns]
if missing:
    raise ValueError(f"df is missing required columns: {missing}")
...
for c in ['STORE_NAME','CUST_CODE']:
    d[c] = d[c].astype(str).str.strip()

# Sale type (optional info column)
sale_type = np.where(
    d.get('CAP_CUSTOMER_CODE', pd.Series('', index=d.index)).astype(str).str.strip().replace({'nan':'','NaN':'','None':''})=='',
    'General sales', 'On_account sales'
)
d['Sale_Type'] = sale_type

# Ensure CASHIER column exists
if 'CASHIER' not in d.columns:
    d['CASHIER'] = ''  # placeholder if missing

# ---- Widgets ----
branch_dd = widgets.Dropdown(
    options=sorted(d['STORE_NAME'].dropna().unique().tolist()),
    description='Branch:',
    layout=widgets.Layout(width='360px')
)
stype_dd = widgets.Dropdown(
...
out = widgets.Output()
display(widgets.HBox([branch_dd, stype_dd, run_btn]), out)
...
def render(_=None):
    with out:
        ...
        dfb = d[(d['STORE_NAME'] == b) & (d['NET_SALES'] < 0)].copy()
        if t != 'All':
            dfb = dfb[dfb['Sale_Type'] == t]

        if dfb.empty:
            print(f"🟢 No negative receipts found for {b} ({t}).")
            return

        # 🔹 Aggregate per unique receipt (sum value, min time, first cashier)
        agg = (
            dfb.groupby(['STORE_NAME','Sale_Type','CUST_CODE'], as_index=False)
               .agg(
        ...
        agg = agg.sort_values('Total_Value')  # more negative first
        ...
        # Totals
        total_receipts = len(disp)
        total_value = disp['Value'].sum()

        # Format
        disp['Time'] = disp['Time'].dt.strftime('%Y-%m-%d %H:%M:%S')
        disp['Value'] = disp['Value'].map(lambda x: f"{x:,.2f}")

        # Add serial #
        disp.insert(0, '#', range(1, len(disp)+1))

        # Output
        print(f"🧾 Negative Receipts (Summed per Unique Receipt) — {b}")
        print(f"Type: {t} | Receipts: {total_receipts:,} | Total Value: {total_value:,.2f}\n")
        display(disp[['#','Branch','Type','Receipt_No','Time','Cashier','Value']])
